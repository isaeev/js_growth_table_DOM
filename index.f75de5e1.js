...
const handleAddRow = ()=>{
    ...
    for(let i = 0; i < columnsCount; i++)newRow.appendChild(document.createElement("td"));
    table.appendChild(newRow);
    removeRow.disabled = false;
    if (rows.length + 1 === 10) appendRow.disabled = true;
};
const handleRemoveRow = ()=>{
    const rows = table.querySelectorAll("tr");
    if (rows.length > 2) table.removeChild(rows[rows.length - 1]);
    appendRow.disabled = false;
    if (rows.length - 1 === 2) removeRow.disabled = true;
};
